...
const LAYER_KIND_COLORS: Record<string, string> = {
  detections:   '#00E5FF',
  segmentation: '#FF4D4D',
  roi:          '#22C55E',
  lesion_token: '#EF4444',
...
export function colorForLabel(label?: string, fallback = '#00FF7F'): string {
  if (!label) {
    return fallback;
  }
  return CLASS_COLORS[label] || fallback;
}
function colorForLayerKind(kind: string): string {
...
export function drawLabeledBoxes(
  ctx: CanvasRenderingContext2D,
...
export async function addLayerToNiivue(
  nv: Niivue | null,
  layer: LayerConfig,
...
declare global {
  interface Window {
    __overlayAdapters?: {
      colorForLabel: typeof colorForLabel;
