...
- **test-image.png**: Sample cervical cytology slide
- **Purpose**: Demo medical image for testing

#### **📊 Mock Data (`public/mock/`)**

**🗂️ case-demo.json** - Demo Case Configuration
```json
{
  "case_id": "DEMO-001",
  "slides": [{
    ...
    "layers": [
      ...
      {"layer_id": "lesion-points", "geometry": "points"}
    ]
  }]
}
```

**🔍 detections-rects.json** - Rectangle Overlays (GeoJSON)
...
**🤖 classify.json** - Mock AI Classification Results
- **Format**: Bounding boxes with labels and confidence scores
- **Purpose**: Simulates real AI classification API response

---

### **🧠 JavaScript Modules (`src/` directory)**

#### **🔌 API Layer (`src/api/`)**

**📡 cqaiClient.js** - Case & Asset Management
```javascript
Functions:
├── fetchCase() - Loads case configurations
├── resolveUri() - Handles mock vs production URLs
└── Environment detection (mock vs API mode)
```
...
3. **Auto-loads**: Demo case (`case-demo.json`) with sample image
...
- **Process**:
  ...
  4. Sets up layer controls in sidebar
- **Result**: Clean demo state with sample medical image
...
#### **📂 "Load" (Sidebar)**
- **Function**: Load custom case from URL
- **Input**: Case JSON URL in text field
- **Use Cases**:
  - `./cases/patient-123.json` - Local case files
  - `https://api.hospital.com/cases/456` - Remote API
  - Research datasets, different patient slides

### **🎚️ Layer Controls (Sidebar)**
- **detections-rects**: Toggle rectangle overlays (blue boxes)
- **lesion-points**: Toggle point markers (red dots)
- **Purpose**: Show/hide different AI analysis layers
...
User Action → UI Event → API Call → Data Processing → Canvas Rendering
     ↓              ↓           ↓            ↓              ↓
Load Demo → fetchCase() → case-demo.json → Layer Loading → Image Display
