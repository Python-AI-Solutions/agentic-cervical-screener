import { resolveUri } from '../api/cqaiClient.js?v=9';
import { classify } from '../api/classifyClient.js?v=9';
import { addLayerToNiivue, boxesFromGeoJSON, colorForLayerKind, drawLabeledBoxes, drawGeoJSON } from './overlayAdapters.js?v=9';
import { collectRois } from './roiNav.js?v=9';
...
let nv=null, rois=[], currentSlideId=null, currentSlideUri=null, currentSlideResolvedUri=null, lastLoadedCase=null;
...
let showGroundTruth = false;         // Toggle for ground truth visibility
...
window.loadCaseFromUrl = async function loadCaseFromUrl(url){
  ...
  showGroundTruth = false; // Ground truth off by default (toggle to enable)
  ...
  let doc=null;
  if (!url) {
    throw new Error('No case URL provided. Select a dataset sample from the sidebar.');
  }
  // Check cache first
  ...
  currentSlideResolvedUri = imgUrl;
  ...
  const layerPromises = (slide.layers||[]).map(async (L) => {
    // Case layers are treated as ground-truth annotations and controlled by the single
    // "ground-truth" toggle (no per-layer toggles to avoid confusion).
    ...
    try {
      ...
      if (fc) {
        ...
        // Redraw overlays so newly loaded layers appear without requiring user interaction.
...
function renderOverlays(){
  ...
  // Show case layers (ground-truth / annotations) if enabled.
  // These are loaded from the case manifest (public/cases/*.json) and cached in layerCache.
  if (showGroundTruth && layerCache.size > 0) {
    for (const cached of layerCache.values()) {
      const gtBoxes = boxesFromGeoJSON(cached.fc);
      if (gtBoxes.length > 0) {
        drawLabeledBoxes(overlayCtx, gtBoxes, transform);
      } else {
        drawGeoJSON(overlayCtx, cached.fc, colorForLayerKind(cached.colorKind), transform);
      }
    }
  }
...
function addGroundTruthToggle() {
  addLayerToggle('ground-truth', 'annotations', false);
}
...
btnClassify.addEventListener('click', async ()=>{
  try {
    ...
    const res=await classify({
      ...
      imageUri: currentSlideResolvedUri || currentSlideUri,
      imageData: currentImageFile,
      imageElement: currentImageObject,
      confThreshold: 0.25
    });
...
setTimeout(() => {
  ...
  async function loadInitialCase() {
    ...
    const hasBackend = typeof env.API_BASE === 'string' && env.API_BASE.length > 0;
    if (!hasBackend) {
      try {
        const res = await fetch('cases/dataset-samples.json', { cache: 'no-store' });
        if (res.ok) {
          const doc = await res.json();
          const first = Array.isArray(doc?.cases) ? doc.cases[0] : null;
          if (first?.file) {
            await loadCaseFromUrl(`cases/${first.file}`);
            return;
          }
        }
      } catch (e) {
        console.warn('Failed to load dataset samples for initial case:', e);
      }
    }
    setStatus('no dataset samples found');
    showSpinner(false);
  }

  loadInitialCase().catch((e) => {
