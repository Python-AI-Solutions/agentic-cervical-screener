...
export function colorForLayerKind(kind) {
...
export function boxesFromGeoJSON(fc) {
  if (!fc || fc.type !== 'FeatureCollection') return [];
  const boxes = [];
  ...
  for (const f of fc.features || []) {
    const g = f?.geometry;
    if (!g) continue;
    if (g.type !== 'Polygon' || !Array.isArray(g.coordinates) || g.coordinates.length === 0) continue;
    ...
    const label = props.label || props.class_name || props.class || props.kind || 'roi';
    const score = typeof props.score === 'number' ? props.score : null;
    const class_id = Number.isFinite(props.class_id) ? Number(props.class_id) : undefined;
